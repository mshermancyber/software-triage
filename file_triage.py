#!/usr/bin/env python3
"""
file_triage.py — First-Level Malware Triage Tool  v2.0
═══════════════════════════════════════════════════════
Hashes a file, classifies application type, extracts metadata/strings/IOCs,
performs PE analysis, queries multiple open-source threat intel APIs, and
...
Usage:
  ...
  python file_triage.py setup.exe --pdf report.pdf
...
def compute_risk_score(circl, vt, mb, urlhaus, threatfox, otx, pe, strings, meta, appcat) -> int:
...
def build_pdf(filepath: str, hashes: dict, meta: dict, appcat: dict,
              ...
              urlhaus: dict, threatfox: dict, otx: dict, risk: int) -> str:
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="file_triage.py v2.0 — First-Level Malware Triage Tool",
...
Examples:
  ...
  python file_triage.py setup.exe --pdf
  python file_triage.py setup.exe --pdf /reports/setup_triage.pdf
